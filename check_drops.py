...
def main():
 ...
 if "hostname" in theJSON["sysinfo"]:
    print "Stats for " +  theJSON["sysinfo"]["hostname"] + "\tpps=Packets Per Second\teps=Errors per Second"
    for stat in theJSON["stats"]:
        print "Iteration Number="+ str(stat["iteration"])
	table = PrettyTable(['Port','switch','vnic_dev','txpps','txpps_drv','txeps','txeps_drv','txmbps','txmbps_drv','rxpps','rxpps_drv','rxeps','rxeps_drv','rxmbps','rxmbps_drv'])
